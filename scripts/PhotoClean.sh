...
ModelList="`dirname $_`"/../share/CameraModels

#Some defaults
PHOTODIR=~/Pictures

# Rename files if true
CLEAN=1

# Check for files not matching camera name
CLEANALL=1

# Move files to new directory if true
MOVE=1
...
do
	...
	{
	cat "${ModelList}" | while read model;
	do
	    U=`echo $model | sed 's/;.*$//'`
